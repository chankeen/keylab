...
export default {
  ...
  created() {},
  components: { uploadFile, selectUser, "tinymce-editor": Editor },
  methods: {
    show(info) {
      console.log(info);
      this.info = JSON.parse(JSON.stringify(info));
      if (this.info.known_date == "0000-00-00") {
        this.info.known_date = null;
      } else {
        this.info.known_date = moment(this.info.known_date, "YYYY-MM-DD");
      }
      if (this.info.deadline == "0000-00-00") {
        this.info.deadline = null;
      } else {
        this.info.deadline = moment(this.info.deadline, "YYYY-MM-DD");
      }
      if (this.info.important_file == null || this.info.important_file == "") {
