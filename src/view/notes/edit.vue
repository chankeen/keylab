      ...
      <a-row>
        <a-col>
          <p class="item">
            ...
            <a-input readonly v-model="info.creation_timestamp"></a-input>
          </p>
          <p class="item">
            <span class="label">最後修改時間</span>
            <a-input readonly v-model="info.last_update_timestamp"></a-input>
          </p>
          <p class="item">
...
export default {
  ...
  methods: {
    show(info) {
      ...
      if (this.info.note_file == null || this.info.note_file == "") {
        this.info.note_file = [];
      }
    ...
    onSubmit() {
      ...
      u_notes(this.handle_submit_data(this.submit_info))
        .then(res => {
          if (res.status) {
